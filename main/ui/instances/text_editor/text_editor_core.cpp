...
/*TextEditorCore::TextEditorCore(const std::filesystem::path& PathToFile)
...
TextEditorCore::TextEditorCore(const std::string &buffer)
    : mLineSpacing(1.0f), mUndoIndex(0), mTabSize(4), mOverwrite(false),
      mReadOnly(false), mWithinRender(false), mScrollToCursor(false),
      mScrollToTop(false), mTextChanged(false), mColorizerEnabled(true),
      mTextStart(20.0f), mLeftMargin(10), mCursorPositionChanged(false),
...
void TextEditorCore::SwitchToFile(std::string PathToFile) {
...
void TextEditorCore::SetLanguageDefinition(
...
void TextEditorCore::SetPalette(const Palette &aValue) {
  mPaletteBase = aValue;
}

std::string TextEditorCore::GetText(const Coordinates &aStart,
...
TextEditorCore::Coordinates TextEditorCore::GetActualCursorCoordinates() const {
  return SanitizeCoordinates(mState.mCursorPosition);
}

TextEditorCore::Coordinates
TextEditorCore::SanitizeCoordinates(const Coordinates &aValue) const {
...
static int UTF8CharLength(TextEditorCore::Char c) {
  if ((c & 0xFE) == 0xFC)
    return 6;
  if ((c & 0xFC) == 0xF8)
    return 5;
...
void TextEditorCore::Advance(Coordinates &aCoordinates) const {
...
void TextEditorCore::DeleteRange(const Coordinates &aStart,
...
int TextEditorCore::InsertTextAt(Coordinates & /* inout */ aWhere,
                                 const char *aValue) {
  assert(!mReadOnly);
...
void TextEditorCore::AddUndo(UndoRecord &aValue) {
  assert(!mReadOnly);
...
TextEditorCore::Coordinates
TextEditorCore::FindWordStart(const Coordinates &aFrom) const {
...
TextEditorCore::Coordinates
TextEditorCore::FindWordEnd(const Coordinates &aFrom) const {
  Coordinates at = aFrom;
  if (at.mLine >= (int)mLines.size())
    return at;
...
TextEditorCore::Coordinates
TextEditorCore::FindNextWord(const Coordinates &aFrom) const {
  Coordinates at = aFrom;
  if (at.mLine >= (int)mLines.size())
    return at;
...
int TextEditorCore::GetCharacterIndex(const Coordinates &aCoordinates) const {
...
int TextEditorCore::GetCharacterColumn(int aLine, int aIndex) const {
...
int TextEditorCore::GetLineCharacterCount(int aLine) const {
  if (aLine >= mLines.size())
    return 0;
  auto &line = mLines[aLine];
  int c = 0;
...
int TextEditorCore::GetLineMaxColumn(int aLine) const {
  if (aLine >= mLines.size())
    return 0;
  auto &line = mLines[aLine];
  int col = 0;
...
bool TextEditorCore::IsOnWordBoundary(const Coordinates &aAt) const {
...
void TextEditorCore::RemoveLine(int aStart, int aEnd) {
  assert(!mReadOnly);
  assert(aEnd >= aStart);
...
void TextEditorCore::RemoveLine(int aIndex) {
  assert(!mReadOnly);
...
TextEditorCore::Line &TextEditorCore::InsertLine(int aIndex) {
  assert(!mReadOnly);
...
std::string TextEditorCore::GetWordUnderCursor() const {
  auto c = GetCursorPosition();
  return GetWordAt(c);
}

std::string TextEditorCore::GetWordAt(const Coordinates &aCoords) const {
...
ImU32 TextEditorCore::GetGlyphColor(const Glyph &aGlyph) const {
  if (!mColorizerEnabled)
...
void TextEditorCore::Render(const char *aTitle, const ImVec2 &aSize,
                            bool aBorder) {
  mWithinRender = true;
  mTextChanged = false;
...
void TextEditorCore::SetText(const std::string &aText) {
  mLines.clear();
...
void TextEditorCore::SetTextLines(const std::vector<std::string> &aLines) {
  mLines.clear();
...
void TextEditorCore::EnterCharacter(ImWchar aChar, bool aShift) {
  assert(!mReadOnly);
      ...
      for (int i = start.mLine; i <= end.mLine; i++) {
        ...
        if (aShift) {
          ...
          line.insert(line.begin(),
                      Glyph('\t', TextEditorCore::PaletteIndex::Background));
          modified = true;
        }
      }
...
void TextEditorCore::SetReadOnly(bool aValue) { mReadOnly = aValue; }

void TextEditorCore::SetColorizerEnable(bool aValue) {
  mColorizerEnabled = aValue;
}

void TextEditorCore::SetCursorPosition(const Coordinates &aPosition) {
...
void TextEditorCore::SetSelectionStart(const Coordinates &aPosition) {
  mState.mSelectionStart = SanitizeCoordinates(aPosition);
  if (mState.mSelectionStart > mState.mSelectionEnd)
    std::swap(mState.mSelectionStart, mState.mSelectionEnd);
}

void TextEditorCore::SetSelectionEnd(const Coordinates &aPosition) {
  mState.mSelectionEnd = SanitizeCoordinates(aPosition);
  if (mState.mSelectionStart > mState.mSelectionEnd)
    std::swap(mState.mSelectionStart, mState.mSelectionEnd);
}

void TextEditorCore::SetSelection(const Coordinates &aStart,
                                  const Coordinates &aEnd,
                                  SelectionMode aMode) {
  ...
  switch (aMode) {
  case TextEditorCore::SelectionMode::Normal:
    break;
  case TextEditorCore::SelectionMode::Word: {
    mState.mSelectionStart = FindWordStart(mState.mSelectionStart);
    if (!IsOnWordBoundary(mState.mSelectionEnd))
      mState.mSelectionEnd = FindWordEnd(FindWordStart(mState.mSelectionEnd));
    break;
  }
  case TextEditorCore::SelectionMode::Line: {
...
void TextEditorCore::SetTabSize(int aValue) {
  mTabSize = (std::max)(0, (std::min)(32, aValue));
}

void TextEditorCore::InsertText(const std::string &aValue) {
  InsertText(aValue.c_str());
}

void TextEditorCore::InsertText(const char *aValue) {
  if (aValue == nullptr)
    return;
...
void TextEditorCore::MoveUp(int aAmount, bool aSelect) {
...
void TextEditorCore::MoveDown(int aAmount, bool aSelect) {
...
void TextEditorCore::MoveLeft(int aAmount, bool aSelect, bool aWordMode) {
  if (mLines.empty())
    return;
...
void TextEditorCore::MoveRight(int aAmount, bool aSelect, bool aWordMode) {
  auto oldPos = mState.mCursorPosition;
...
void TextEditorCore::MoveTop(bool aSelect) {
  auto oldPos = mState.mCursorPosition;
...
void TextEditorCore::TextEditorCore::MoveBottom(bool aSelect) {
...
void TextEditorCore::MoveHome(bool aSelect) {
  auto oldPos = mState.mCursorPosition;
...
void TextEditorCore::MoveEnd(bool aSelect) {
  auto oldPos = mState.mCursorPosition;
...
bool TextEditorCore::CanUndo() const { return !mReadOnly && mUndoIndex > 0; }

bool TextEditorCore::CanRedo() const {
  return !mReadOnly && mUndoIndex < (int)mUndoBuffer.size();
}

void TextEditorCore::Undo(int aSteps) {
  while (CanUndo() && aSteps-- > 0)
    mUndoBuffer[--mUndoIndex].Undo(this);
}

void TextEditorCore::Redo(int aSteps) {
...
const TextEditorCore::Palette &TextEditorCore::GetLightPalette() {
  const static Palette p = {{
      0xff7f7f7f, // None
      0xffff0c06, // Keyword
      0xff008000, // Number
...
const TextEditorCore::Palette &TextEditorCore::GetRetroBluePalette() {
  const static Palette p = {{
      0xff00ffff, // None
      0xffffff00, // Keyword
      0xff00ff00, // Number
...
std::vector<std::string> TextEditorCore::GetTextLines() const {
...
std::string TextEditorCore::GetCurrentLineText() const {
...
void TextEditorCore::ColorizeRange(int aFromLine, int aToLine) {
...
void TextEditorCore::EnsureCursorVisible() {
  if (!mWithinRender) {
...
TextEditorCore::UndoRecord::UndoRecord(
    const std::string &aAdded, const TextEditorCore::Coordinates aAddedStart,
    const TextEditorCore::Coordinates aAddedEnd, const std::string &aRemoved,
    const TextEditorCore::Coordinates aRemovedStart,
    const TextEditorCore::Coordinates aRemovedEnd,
    TextEditorCore::EditorState &aBefore, TextEditorCore::EditorState &aAfter)
...
void TextEditorCore::UndoRecord::Undo(TextEditorCore *aEditor) {
  if (!mAdded.empty()) {
...
void TextEditorCore::UndoRecord::Redo(TextEditorCore *aEditor) {
  if (!mRemoved.empty()) {
...
TextEditorCore::LanguageDefinition::CPlusPlus() {
...
const TextEditorCore::LanguageDefinition &
TextEditorCore::LanguageDefinition::HLSL() {
  static bool inited = false;
  static LanguageDefinition langDef;
  if (!inited) {
...
const TextEditorCore::LanguageDefinition &
TextEditorCore::LanguageDefinition::GLSL() {
  static bool inited = false;
  static LanguageDefinition langDef;
  if (!inited) {
    static const char *const keywords[] = {
...
const TextEditorCore::LanguageDefinition &
TextEditorCore::LanguageDefinition::C() {
  static bool inited = false;
  static LanguageDefinition langDef;
  if (!inited) {
    static const char *const keywords[] = {
...
const TextEditorCore::LanguageDefinition &
TextEditorCore::LanguageDefinition::SQL() {
  static bool inited = false;
  static LanguageDefinition langDef;
  if (!inited) {
...
const TextEditorCore::LanguageDefinition &
TextEditorCore::LanguageDefinition::AngelScript() {
  static bool inited = false;
  static LanguageDefinition langDef;
  if (!inited) {
    static const char *const keywords[] = {
...
const TextEditorCore::LanguageDefinition &
TextEditorCore::LanguageDefinition::Lua() {
  static bool inited = false;
  static LanguageDefinition langDef;
  if (!inited) {
    static const char *const keywords[] = {
